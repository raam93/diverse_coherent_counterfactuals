...
class linear_explanation:
    ...
    def encode_pandas(self, inputs):
        ...
        # at this point, all dataframe values are numbers
        for i in range(inputs.columns.shape[0]):
            # get all col values
            col=inputs[inputs.columns[i]]
            # collect all the negative values (if pos, replace w/ 0)
            vals=np.minimum(col,0)
            # collect all non-neg values, normalize
            normal=col[(col>=0)]
            scale=normal.max()-normal.min()
            minx = normal.min()
            ...
            normal=(normal-minx)/scale
            # get list of unique negative vals
            un=np.unique(vals)
            un=un[un!=0]
            # encode cont range (MAD scale), dummy-encode discrete
            tab=np.zeros((un.shape[0]+1,vals.shape[0]))
            # [0] is continuous range
            tab[0]=np.maximum((col-minx)/scale,0)
            ...
            # typo, bug? doesn't seem to ever be true, in any case.
            # ...unless is nan, but that's caught below.
            if not(MAD[0]>0):
                MAD[0]==1
            if np.isnan(MAD[0]):
                MAD[0]=1
            ...
            j=1
            for u in un:
                # ''MAD'' for discrete vars (std)
                tab[j]=(col==u)
                med[j]=np.mean(tab[j])
                MAD[j]=1.48*np.std(tab[j])
                if not(MAD[j]>0):
                    MAD[j]==1e-4
                j+=1
            # create a new class for each column
            self.cox[i]=SimpleNamespace(name=inputs.columns[i],total=tab, #normal=normal,
                                   med=med,MAD=1.0/MAD,unique=un,scale=scale,minx=minx)
    ...
    def train_logistic(self,target,subset=False):
        ...
        self.coef_ = np.array([[ 2.4019535 ,  3.1055427 ,  0.26152474, -1.114933  ,  0.04414963,
        -0.10692964, -0.41944346,  0.5381504 ,  1.7977493 , -0.6286769 ,
         1.1291502 ,  1.6584185 , -1.6776586 , -0.16970578, -0.34199902,
         2.0079951 , -0.14508529, -0.51205504,  0.02133017, -0.13252994,
         0.7326947 ,  0.69880915,  0.45294258,  0.14767966,  0.8151559 ,
         0.09748572,  0.19950235, -0.05005082,  0.35756823]],
      dtype=np.float32)

        self.intercept_ = np.array([-4.5862765], dtype=np.float32)
    ...
    def set_factual(self,factual):
        ...
        for i in range(factual.size):
            if factual[i]>0:
                self.short_factual[i]= (self.short_factual[i]-self.cox[i].minx)/self.cox[i].scale
    ...
    def mixed_encode(self,test_eg):
        ...
        for i in range(self.cox.shape[0]):
            ...
            if test_eg[i]<0:
                ...
                out[index]=(test_eg[i]-c.minx)/c.scale
    ...
    def pp(self,x,other,i):
        ...
        if (x-other<0):
            return "%d"%(np.floor((x*self.cox[i].scale)+self.cox[i].minx))
        return "%d"%(np.ceil((x*self.cox[i].scale)+self.cox[i].minx))

    def explain(self,cf,labels=("'pos'","'neg'")):
        test_entry=self.short_factual
    ...
    def explain_follow(self,cf,labels=("'pos'","'neg'")):
        test_entry=self.short_factual
        cf_score=labels[int(not(self.direction>0))]
        out=cf
        mask= np.abs(out- test_entry)>0.001
    ...
    def give_explanation(self,upto=10,labels=("'pos'","'neg'")):
    ...
    def explain_entry(self,entry,upto=10,labels=("'pos'","'neg'")):
